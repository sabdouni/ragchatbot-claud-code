...
class TestConfig:
    ...
    def test_config_validation_chunk_size_reasonable(self):
        ...
        config = Config()
        assert 100 <= config.CHUNK_SIZE <= 2000, "Chunk size should be between 100-2000 characters"
    
    def test_config_validation_chunk_overlap_smaller_than_size(self):
        """Test that chunk overlap is smaller than chunk size"""
        config = Config()
        assert config.CHUNK_OVERLAP < config.CHUNK_SIZE, "Overlap must be smaller than chunk size"
...
class TestConfigIssues:
    ...
    def test_zero_max_results_issue(self):
        ...
        assert config.MAX_RESULTS != 0, "MAX_RESULTS=0 causes search to return no results"
        assert config.MAX_RESULTS >= 1, "MAX_RESULTS must be at least 1 for meaningful search"
    ...
    def test_model_name_format(self):
        ...
        assert len(config.ANTHROPIC_MODEL) > 6, \
            "Model name seems too short"
...
class TestConfigDefaults:
    ...
    def test_chunk_size_optimization(self):
        ...
        assert 500 <= config.CHUNK_SIZE <= 1200, \
